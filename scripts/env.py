...
class PortfolioEnv(gym.Env):
    ...
    def step(self, action):
        ...
        portfolio_return = np.dot(self.allocations, asset_returns) * 0.875
        noise_factor = np.random.normal(loc=0, scale=0.065)  
        portfolio_return += noise_factor
        ...
        withdrawal_amounts = {6: 52525, 16: 104500}  # Define withdrawals (year: amount)
        if self.current_year + 1 in withdrawal_amounts:  # Check if current year requires a withdrawal
            withdrawal = withdrawal_amounts[self.current_year + 1]
            self.portfolio_value -= withdrawal
            self.portfolio_value = max(self.portfolio_value, 0)  # Ensure portfolio doesn't go negative
